...
type PortfolioItem = {
  ...
  type?: string;
  thumbnail?: string;
  classment?: string; // Added for sorting
};
...
const getFilteredItems = () => {
  ...
  // Separate items with and without classement
  const withClassment = filtered.filter((item) => item.classement);
  const withoutClassment = filtered.filter((item) => !item.classement);

  // Sort classed items numerically (#1 → #99)
  const sortedWithClassment = [...withClassment].sort((a, b) => {
    const numA = parseInt(a.classement.replace("#", ""), 10);
    const numB = parseInt(b.classement.replace("#", ""), 10);
    return numA - numB;
  });

  // Merge: classed ones first, then the rest in original order
  const final = [...sortedWithClassment, ...withoutClassment];

  return showAll ? final : final.slice(0, 8);
};


  const filteredItems = useMemo(() => getFilteredItems(), [activeFilter, showAll]);
          ...
          {filters.map((filter) => (
            <div key={filter} className="flex items-center">
              <button
                ...
                className={`px-2 py-2 text-sm md:text-2xl transition-colors font-bold ${
        ...
        <AnimatePresence>
          {finalItems.map((item, index) => {
            ...
            return (
              ...
              >
                {activeFilter === 'Production vidéo' || item.type === 'video' ? (
                  <HoverVideo
                    src={item.image}
                    alt={item.title}
                  ...
                  <img
                    src={activeFilter === 'Web' && item.webImage ? item.webImage : item.image}
                    alt={item.title}
